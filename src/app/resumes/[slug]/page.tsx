...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const guide = await prisma.guide.findFirst({ where: { slug: params.slug, kind: "resume", isPublished: true } });
  if (!guide) return { title: "Резюме не найдены", robots: { index: false, follow: false } };
    ...
    <SeoLandingPage landing={landing}>
      <section className="border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3">
          ...
          {resumes.map((item) => (
            <ResumeDirectoryCard key={item.id} resume={item} currentPath={landing.path} />
          ))}
        </div>
      </section>
    </SeoLandingPage>
